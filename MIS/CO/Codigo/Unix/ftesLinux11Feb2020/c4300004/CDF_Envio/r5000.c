...
int err_handler(DBPROCESS *dbproc, int severity, int dberr, int oserr, char *dberrstr, char *oserrstr)
{
   if (dberr==SYBETIME)
   {
      fprintf (stderr, "Error por timeout de DB-Library:\n\t%s\n", dberrstr);
      return (INT_TIMEOUT);
   }
   if (dberr==SYBESMSG)
      return (INT_CANCEL);
   ...
   {
     fprintf (stderr, "Error de DB-Library:\n\t%s\n", dberrstr);
     if (oserr != DBNOERR)
       fprintf (stderr, "Error de Systema Operativo:\n\t%s\n", oserrstr);
     return(INT_EXIT);
   }
}
...
{
   fprintf (stderr, "Msg %d, Nivel %d, Estado %d\n", msgno, severity, msgstate);
   if (strlen(srvname) > 0)
     fprintf (stderr, "Server '%s', ", srvname);
   if (strlen(procname) > 0)
     fprintf (stderr, "Proceso '%s', ", procname);
   if (line > 0)
     fprintf (stderr, "Nivel de linea %d", line);
   fprintf (stderr, "\n\t%s\n", msgtext);
}
   return(0);
}
...
char cadenareg5000[615]; /*se agregan 54 espacios para tabulador*/
...
totalMonUsu = contadorMonUsu;
 ...
 /**********************************************************************************/
   ...
   strcpy(sqlcmd," select distinct CtaCorp=convert(char(16), convert(char(4), \n");
   ...
   if (strcmp(cortenum,"0") == 0)  /* Proceso dirario o por rango de fechas */
   {
    ...
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(fecIni)) );
    sprintf(sqlcmd2," %s %s ", sqlcmd,fecIni);
    strcpy(sqlcmd, sqlcmd2);
    strcat(sqlcmd," and \n");
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(fecFin)) );
    sprintf(sqlcmd2," %s %s ", sqlcmd,fecFin);
    strcpy(sqlcmd, sqlcmd2);
   }
   if (strcmp(cortenum,"0") != 0) /* Proceso por corte */
   {
    strcat(sqlcmd," A.his_mes_y_ciclo_corte in \n");
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(corte)) );
    sprintf(sqlcmd2," %s (%s) ", sqlcmd,corte);
    strcpy(sqlcmd, sqlcmd2);
   }
   if (strcmp(ejePrefijo,"0") != 0) /* Valida si se toma prefijo para proceso */
   {
    strcat(sqlcmd,"and A.eje_prefijo= \n");
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(ejePrefijo)) );
    sprintf(sqlcmd2," %s %s ",sqlcmd,ejePrefijo);
    strcpy(sqlcmd, sqlcmd2);
   }
   if (strcmp(gpoBanco,"0") !=0) /* Valida si se tom banco para proceso */
   {
    strcat(sqlcmd,"and A.gpo_banco= \n");
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(gpoBanco)) );
    sprintf(sqlcmd2," %s %s ",sqlcmd,gpoBanco);
    strcpy(sqlcmd, sqlcmd2);
   }
   if (strcmp(numEmpresa,"0") !=0) /* Valida si se toma empresa para proceso */
   {
    strcat(sqlcmd,"and A.emp_num= \n");
    sqlcmd2 = (char*)malloc( (strlen(sqlcmd) + strlen(numEmpresa)) );
    sprintf(sqlcmd2," %s %s ",sqlcmd,numEmpresa);
    strcpy(sqlcmd, sqlcmd2);
   }
...
*/
     ...
     memset(addendumtype,'\0',sizeof(addendumtype));
     ...
     memset(merchantacttranoriginind,'\0',sizeof(merchantacttranoriginind));
     memset(accountnumber,'\0',sizeof(accountnumber));
     memset(reservedforinternaluse3,'\0',sizeof(reservedforinternaluse3));
     memset(acquirersorissuersrefnum,'\0',sizeof(acquirersorissuersrefnum));
     memset(recordtype,'\0',sizeof(recordtype));
     memset(transactiontype,'\0',sizeof(transactiontype));
     memset(debitcreditindicator,'\0',sizeof(debitcreditindicator));
     memset(transactionamount,'\0',sizeof(transactionamount));
     memset(postingdate,'\0',sizeof(postingdate));
     memset(transactiondate,'\0',sizeof(transactiondate));
     memset(processingdate,'\0',sizeof(processingdate));
     memset(merchantname,'\0',sizeof(merchantname));
     memset(merchantcity,'\0',sizeof(merchantcity));
     memset(merchantstateprovince,'\0',sizeof(merchantstateprovince));
     memset(merchantcountry,'\0',sizeof(merchantcountry));
     memset(merchantcategorycode,'\0',sizeof(merchantcategorycode));
     memset(amountinoriginalcurrency,'\0',sizeof(amountinoriginalcurrency));
     memset(originalcurrencycode,'\0',sizeof(originalcurrencycode));
     memset(currconvdatejulian,'\0',sizeof(currconvdatejulian));
     memset(postedcurrencycode,'\0',sizeof(postedcurrencycode));
     memset(conversionrate,'\0',sizeof(conversionrate));
     memset(conversionexponent,'\0',sizeof(conversionexponent));
     memset(acquiringica,'\0',sizeof(acquiringica));
     memset(customercode,'\0',sizeof(customercode));
     memset(salestaxamount,'\0',sizeof(salestaxamount));
     memset(reservedforinternaluse4,'\0',sizeof(reservedforinternaluse4));
     memset(reservedforinternaluse5,'\0',sizeof(reservedforinternaluse5));
     memset(freightamount,'\0',sizeof(freightamount));
     memset(destinationpostalcode,'\0',sizeof(destinationpostalcode));
     memset(merchanttype,'\0',sizeof(merchanttype));
     memset(merchantlocationpostalcode,'\0',sizeof(merchantlocationpostalcode));
     memset(dutyamount,'\0',sizeof(dutyamount));
     memset(merchantfederaltaxid,'\0',sizeof(merchantfederaltaxid));
     memset(merchantstateprovincecode,'\0',sizeof(merchantstateprovincecode));
     memset(shipfrompostalcode,'\0',sizeof(shipfrompostalcode));
     memset(alternatetaxamount,'\0',sizeof(alternatetaxamount));
     memset(destinationcountrycode,'\0',sizeof(destinationcountrycode));
     memset(merchantreferencenumber,'\0',sizeof(merchantreferencenumber));
     memset(alternatetaxindicator,'\0',sizeof(alternatetaxindicator));
     memset(alternatetaxidentifier,'\0',sizeof(alternatetaxidentifier));
     memset(salestaxcollectedind_pos,'\0',sizeof(salestaxcollectedind_pos));
     memset(addendumdetailindicator,'\0',sizeof(addendumdetailindicator));
     memset(merchantid,'\0',sizeof(merchantid));
     memset(banknetReferenceNumber, '\0',sizeof(banknetReferenceNumber)); /*Nuevo Campo, IERM Sept. 15, 2004*/
     memset(approvalCode, '\0',sizeof(approvalCode));  /*Nuevo Campo, IERM Sept. 15, 2004*/
     memset(adjustmentreasoncode,'\0',sizeof(adjustmentreasoncode));
     memset(adjustmentdescription,'\0',sizeof(adjustmentdescription));
     memset(maintenanceactioncode,'\0',sizeof(maintenanceactioncode));
     ...
     memset(valor_mcc,'\0',sizeof(valor_mcc));
     memset(cadenareg5000,'\0',sizeof(cadenareg5000));
...
/*     sprintf(conversionrate, FRMN16, EMPTYSTR);*/
     sprintf(conversionrate, "%016ld", atol(EMPTYSTR));
...
/*       sprintf( acquiringica, FRMN11,  issuerIca  ); */  /*Formateado, IERM Sept. 21, 2004*/
       sprintf( acquiringica, "%011ld",  atol(issuerIca)  );
...
/*        sprintf(valor_mcc, FRMN4, b06Acd); */ /*valor obtenido de MTCNEG01*/
        sprintf(valor_mcc, "%04ld", atol(b06Acd) ); 
...
/*      sprintf( acquiringica, FRMN11, referencia_a );*/
      sprintf( acquiringica, "%011ld", atol(referencia_a) );
...
/*         sprintf( amountinoriginalcurrency, FRMN16, importe1 );*/ /*cuatro decimales sin punto*/
         sprintf( amountinoriginalcurrency, "%016ld", atol(importe1) );
       else
/*        sprintf( amountinoriginalcurrency, FRMN16, dolares1); */ /*cuatro decimales sin punto*/
        sprintf( amountinoriginalcurrency, "%016ld", atol(dolares1));
...
/*      sprintf( salestaxamount, FRMN16, EMPTYSTR );*/
      sprintf( salestaxamount, "%016ld", atol(EMPTYSTR) );
...
/*      sprintf( freightamount, FRMN16, EMPTYSTR );*/
      sprintf( freightamount, "%016ld", atol(EMPTYSTR) );
...
/*      sprintf( dutyamount, FRMN16, EMPTYSTR );*/
      sprintf( dutyamount, "%016ld", atol(EMPTYSTR) );
...
/*      sprintf( alternatetaxamount, FRMN16, EMPTYSTR );*/
      sprintf( alternatetaxamount, "%016ld", atol(EMPTYSTR) );
...
/*      if ( (strcmp(orgpointnum,"0") == 0) || (strcmp(orgpointnum,NULL) == 0) || (strcmp(orgpointnum," ") == 0) ) */
       if ( (strcmp(orgpointnum,"0") == 0) || (orgpointnum[0] == '\0') || (strcmp(orgpointnum," ") == 0) )
       {
...
*/
     ...
     /* Se agrega set de caracteres */
     strcat(bulkcopy, "  -Jroman8");
