...
/*    argv[10] = Fecha1 de Procesamiento (Opcional).                          */
/*    argv[11] = Fecha2 de Procesamiento (Opcional).                          */
...
#endif                                       
{
   ...
   if(!((argc == 7) || (argc == 10) || (argc == 12)))   
   {
      printf("\nUse: C430TransCCF <User> <Password> <Server>\n");
      printf("                  <File Type> <File Mode> <File Name>\n");
      printf("                  [<Preffix> <Bank Group> <Company ID> <ProcessDate1> <ProccessDate2]\n");
      exit(ERREXIT);
   }
...
/*    psParams[10] = Fecha1 de Procesamiento (Opcional).                      */
/*    psParams[11] = Fecha2 de Procesamiento (Opcional).                      */
...
#endif                                       
{
 ...
 /*estas variales agrega yuria para recuperar datos de pais*/
      ...
      char sProcessDate1[LEN8+1];   /* Fecha1 de Procesamiento de Datos. */ 
      char sProcessDate[LEN8+1];   /* Fecha2 de Procesamiento de Datos. */ 
   ...
   /* Respeta tal cual las fechas que pasaron          */
   if ( strlen(psParams[10]) > 0)
      strcpy(sProcessDate1,psParams[10]);

   if(strlen(psParams[11]) > 0)
   {
      strcpy(sSystemDate, psParams[11]);
      strcpy(sASystemDate, psParams[11]);
   }
    printf ("fecha1=%s  fecha2=%s \n",sProcessDate1,sSystemDate);
   ...
   for(iCompCount=0; iCompCount < iNumCompanies; iCompCount++)
   {
	 ...
	 ***/
      ...
      else
      {
         ...
         strcat(sSQLCommand, "\' 00:00:00\') BETWEEN ");
         strcat(sSQLCommand, "convert(datetime, \'");
         strcat(sSQLCommand, sProcessDate1);
         strcat(sSQLCommand, "\' + \' 00:00:00\') AND ");
         strcat(sSQLCommand, "convert(datetime, \'");
         strcat(sSQLCommand, sSystemDate);
         strcat(sSQLCommand, "\' + \' 23:59:59\') ");
...
/*                              printf("\n%s\n", sSQLCommand);*/
                              ...
                              strcpy(sSQLCommand, "SELECT signo_transaccion,");
                              strcat(sSQLCommand, "        tip_transaccion  ");
...
#else
char *psElimChar(psString, cCharacter)
char *psString;
char cCharacter;
#endif
{
...
#endif
{
   ...
   strcpy(sLastName, psGetLastName(psPlaName));

   /* Extrae los Apellidos Paterno y Materno del parametro psName. */
 
   if((psString = strstr(psName, sLastName)) != NULL)
   {                                            
      strcpy(sLastNames, psString);                    
   }                 

   /* Copia a la estructura los primeros 25 caracteres (unicamente). */
 
   strncpy(sTemp, sLastNames, LEN25);
   sTemp[LEN25] = EOLN;
   sprintf(pRec2200->sLastName, FMTSLLONG25, sTemp);

   /* Recupera del parametro psName el (los) Nombre(s) (sin Apellidos) del 
      Tarjetahabiente. */

   GetNames(sNames, psName, sLastName, &iNumNames, SPACE);


   /* Establece los valores del Primer Nombre y Segundo Nombre de acuerdo
      al No. Total de valores recuperados en sNames. */

   strncpy(sTemp, sNames[0], LEN25);
   sTemp[LEN25] = EOLN;
   sprintf(pRec2200->sCardHFirstN, FMTSLLONG25, sTemp);

   if(iNumNames <= 1)
   {  
      sprintf(pRec2200->sCardHMidN, FMTSLLONG20, EMPSTR);
   }
   else
   {
      strcpy(sTemp, EMPSTR);

      for(iCounter = 1; iCounter < iNumNames; iCounter++)
      {
         /* Verificar que la cadena a adicionar no exceda la longitud
            maxima del campo. */

         if(strlen(sTemp) + strlen(sNames[iCounter]) >= LEN20)

            break;

         else
         { 
            strcat(sTemp, sNames[iCounter]);
            strcat(sTemp, " ");
         }
      }
...
/*inicia codigo yuria para MCC,SocurceAmt,Merchant Country, State Country, tasa Conversion */
  ...
  /*Si la transaccion es nacional */            
      ...
      (strcmp(psMerchCtry, "") == 0))
   {
     ...
     {  
        if ( ( strcmp(psTraType,"abono")==0 ) ||
             ( strcmp(psTraType,"devolucion")==0 ) ||
   ...
   else /* si la transaccion es internacional*/
   {
      ...
      else
      {
        ...
        {
          if(strcmp(psTraType,"devolucion")==0)
             strcpy(pRec5500->sMCC,"0000");
        }
      }
