...
typedef struct stpParms stPParms;      
...
#endif
{
	...
	do {
                if (pParmsIni==NULL)
                {
                   pParms=(stPParms *)malloc(sizeof(stPParms));
                   if (pParms == NULL )
                   {
                     printf("NO se pudo asignar memoria a pParms al inicio\n");
		     exit(FAILURE);
                   }
                   else
                   {
                ...
                else
                {
                   ...
                   if (pParms == NULL )
                   {
                     printf("NO se pudo asignar memoria a pParms\n");
                     pParmsAnt=pParmsIni;
                     while(pParmsAnt != NULL)
                     {
                       pParmsIni=pParmsAnt;
                       pParmsAnt=pParmsIni->sig;
                       free(pParmsIni);
                     }
		     exit(FAILURE);
                   }
...
/*YURIA*/
		...
		ObtenCampo(&stSQL, (int)  0, (char *) pParms->Parms.sPrefijo);
		ObtenCampo(&stSQL, (int)  1, (char *) pParms->Parms.sBanco);
		ObtenCampo(&stSQL, (int)  2, (char *) pParms->Parms.sEmpresa);
		ObtenCampo(&stSQL, (int)  3, (char *) pParms->Parms.sUnidad);
		ObtenCampo(&stSQL, (int)  4, (char *) pParms->Parms.sReporte);
		ObtenCampo(&stSQL, (int)  5, (char *) pParms->Parms.sFrecuencia);
		ObtenCampo(&stSQL, (int)  6, (char *) pParms->Parms.sLongEmpresa);
		ObtenCampo(&stSQL, (int)  7, (char *) pParms->Parms.sLongEjecutivo);
		ObtenCampo(&stSQL, (int)  8, (char *) pParms->Parms.sLongUnidad);
		ObtenCampo(&stSQL, (int)  9, (char *) pParms->Parms.sLongReporte);
		ObtenCampo(&stSQL, (int) 10, (char *) pParms->Parms.iNivelIni);
		ObtenCampo(&stSQL, (int) 11, (char *) tipo);
		ObtenCampo(&stSQL, (int) 12, (char *) grupo);
           printf("Prefijo %s, Banco %s, Empresa %s, Unidad %s Reporte %s\n",pParms->Parms.sPrefijo, pParms->Parms.sBanco,pParms->Parms.sEmpresa,pParms->Parms.sUnidad,pParms->Parms.sReporte);
...
/*} fi UNIDAD <=LENUNI
        ...
        (void) fprintf(stderr, "lin 592 \n");
			...
			sprintf(Parms.sFechaFinAMD, "%s", "00000000");
                        ...
                        respuesta=SVCREP13(14, argRep);
                        if ( respuesta == 0 )
                        {
            /*fprintf(stdout, "fallo el cp file_c.tmp %s\n",Parms.sArchivoDisco);*/
	...
	switch (atoi(Parms.sReporte)) {
		...
		case 18:
			...
			sprintf(Parms.sFechaFinAMD, "%s", "00000000");
                        ...
                        strcpy(argRep[5], Parms.sEmpresa);
                        strcpy(argRep[6], Parms.sUnidad);
                        strcpy(argRep[7], Parms.sFechaCorteMD);
                        strcpy(argRep[8], Parms.iNivelIni);
                        strcpy(argRep[9], Parms.iNivelFin);
                        strcpy(argRep[10], Parms.sArchivoTemp);
                        strcpy(argRep[11], Parms.sbGrupo);
                        strcpy(argRep[12], Parms.sbTotales);
                        respuesta=SVCREP18(13, argRep);
                        if ( respuesta == 0 )
                        {
                         sprintf(comando, "cp file_c.tmp %s",Parms.sArchivoDisco);
                         system(comando);
		...
		rsql = RegistroSiguiente(&stSQL);
           ...
           /*al final debe liberar la memoria que utilizo para cargar toda la memoria utilizada */
                     pParmsAnt=pParmsIni;
                     while(pParmsAnt != NULL)
                     {
                       pParmsIni=pParmsAnt;
