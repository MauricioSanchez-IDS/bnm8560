...
/*		estado = envia_ftp(	reportes[i].archivo_origen,
                ...
                estado = copia_arcBancaNet(reportes[i].archivo_origen,
                                           reportes[i].archivo_destino);
...
int copia_arcBancaNet(arch1, arch2)
char *arch1;
char *arch2;
{
        int pid, estado, ret;
        ...
        switch (pid) {
                ...
                case 0:
                        ...
                        sprintf(comando, "%s %s %s/%s",
                                         "cp", arch1, "/opt/c430/000/bin/inte/envio/bin/temp/Corporativa", arch2);

                        ret = system(comando);

                        if (ret != 0)
                           printf("Error a realizar comando de copiado de archivo a BancaNet, ret : %d \n",ret);
