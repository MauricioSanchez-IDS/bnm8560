...
echo "sp_IntegracionFase1_TAR $1" > $DIR_TEMP/inteTAR1.sql
echo "go"       >> $DIR_TEMP/inteTAR1.sql
echo "EOF"      >> $DIR_TEMP/inteTAR1.sql
...
# 3. Crea la estructura de la tabla TAR
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteTAR1.sql  | grep -c "return status = 0" `
...
# 1. Se divide el archivo original DTARMMDD en varios archivos con csplit
# 2. se genera un archivo por cada producto ejem: DTARMMDD01, DTARMMDD02, etc.
# 3. Se genera un formato de carga para cada uno de estos archivos a partir
#    del original TAR.fmt, ejem: TAR01.fmt, TAR02.fmt, etc.
...
do
  ...
  then
     ...
     isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/error.sql/bcp
     
     exit 0
  fi
  eje=`echo $resultado | cut -c4-4`
  emp=`echo $resultado | cut -c5-5`
  awk -v emp=$emp -v eje=$eje -f $DIR_CARGA/cambia.awk <$DIR_PARAM/TAR.fmt >$DIR_TEMP/TAR$consecutivo.fmt
done
...
# 1. hace un bulk copy por cada uno de los archi: DTARMMDD01, DTARMMDD02, etc.
# 3. considera los formatos correspondientes:  TAR01.fmt, TAR02.fmt, etc.
cont2=0
...
#bcp $(base.sh)..CARTAR01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/TAR${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
bcp64 $(base.sh)..CARTAR01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/TAR${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt -Jroman8
#_HP_Changes for Linux Migration -End
   ...
   #como termino el bulk copy porque TAR trae un problema aunque 
...
echo "sp_IntegracionFase2_TAR $1" > $DIR_TEMP/inteTAR2.sql
echo "go"       >> $DIR_TEMP/inteTAR2.sql
echo "EOF"      >> $DIR_TEMP/inteTAR2.sql

#---------- Ejecuta el Stored Procedure de 2a. Fase de Integracisn, que incluye:
#1.- Si existe la tabla CARTAR01, inserta los registros de esta a MTCTHS01
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteTAR2.sql  | grep -c "return status = 0" `
