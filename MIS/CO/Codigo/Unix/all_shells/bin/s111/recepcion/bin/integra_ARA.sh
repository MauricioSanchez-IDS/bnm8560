...
echo "sp_IntegracionFase1_ARA $1" > $DIR_CARGA/inteARA1.sh
echo "go"       >> $DIR_CARGA/inteARA1.sh
echo "EOF"      >> $DIR_CARGA/inteARA1.sh
...
#	3. Crea la estructura de la tabla ARA
proc_OK=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_CARGA/inteARA1.sh  | grep -c "return status = 0" `
...
#bcp $(base.sh)..CARARA01 in $DIR_ARCH2/$1 -Q -f$DIR_PARAM/ARA.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
bcp64 $(base.sh)..CARARA01 in $DIR_ARCH2/$1 -Q -f$DIR_PARAM/ARA.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt -Jroman8
...
echo "sp_IntegracionFase2_ARA $1" > $DIR_CARGA/inteARA2.sh
echo "go"       >> $DIR_CARGA/inteARA2.sh
echo "EOF"      >> $DIR_CARGA/inteARA2.sh

#---------- Ejecuta el Stored Procedure de 2a. Fase de Integracisn, que incluye:
# 	1. Si existe la tabla de PRODUCCION MTCARA01, la borra
#     2. Renombra la de CARGA por la de PRODUCCION MTCARA01
#	3. Crea los mndices y las llaves primarias o secundarias, si existen
proc_OK=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_CARGA/inteARA2.sh  | grep -c "return status = 0" `
