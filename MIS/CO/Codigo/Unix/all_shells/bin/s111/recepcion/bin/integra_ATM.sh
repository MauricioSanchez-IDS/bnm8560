...
echo "DIR_ARCH2 : $DIR_ARCH2"
echo "DIR_TEMP : $DIR_TEMP"
echo "DIR_PARAM : $DIR_PARAM"
...
echo "sp_IntegracionFase1_ATM $1" > $DIR_TEMP/inteATM1.sql
echo "go"       >> $DIR_TEMP/inteATM1.sql
echo "EOF"      >> $DIR_TEMP/inteATM1.sql
...
# 3. Crea la estructura de la tabla ATM
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteATM1.sql  | grep -c "return status = 0" `
...
# 1. Se divide el archivo original EATMMMDD en varios archivos con csplit
# 2. se genera un archivo por cada producto ejem: EATMMMDD01, EATMMMDD02, etc.
# 3. Se genera un formato de carga para cada uno de estos archivos a partir
#    del original ATM.fmt, ejem: ATM01.fmt, ATM02.fmt, etc.
...
do
  ...
  awk -v emp=$emp -v eje=$eje -f $DIR_CARGA/cambia.awk <$DIR_PARAM/ATM.fmt >$DIR_TEMP/ATM$consecutivo.fmt
done
...
# 1. hace un bulk copy por cada uno de los archi: EATMMMDD01, EATMMMDD02, etc.
# 3. considera los formatos correspondientes:  ATM01.fmt, ATM02.fmt, etc.
cont2=0
...
#bcp $(base.sh)..CARATM01 in $DIR_ARCH2/${1}${consecutivo} -b 10 -Q -f$DIR_TEMP/ATM${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
bcp64 $(base.sh)..CARATM01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/ATM${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.tx 
...
echo "sp_IntegracionFase2_ATM $1" > $DIR_TEMP/inteATM2.sql
echo "go"       >> $DIR_TEMP/inteATM2.sql
echo "EOF"      >> $DIR_TEMP/inteATM2.sql

#---------- Ejecuta el Stored Procedure de 2a. Fase de Integracisn, que incluye:
#1. Si existe la tabla de PRODUCCION MTCATM01, la borra
#2. Renombra la de CARGA por la de PRODUCCION MTCATM01
#3. Crea los mndices y las llaves primarias o secundarias, si existen
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteATM2.sql  | grep -c "return status = 0" `
