...
USER=c4300001						#Usuario de acceso
...
for registro in `cat "$CONFIL"`
  do
	new_reg=`echo "$registro" | awk '{FS=",[ \t]*|[ \t]+"}{print $1,$2,$3,$4}'`
...
echo "$FORMATOINI Host destino       : $IPORIG">>$LogIntelar
...
echo "cd $BUZON" > $TMP
if [ $COMANDO = "put" ]; then
		echo "put $FILE $FILEDES" >> $TMP	#Asigna comando de Envio
		TIPOOPER="Uploading"
elif [ $COMANDO = "get" ]; then
		echo "get $FILE $FILEDES" >> $TMP	#Asigna comando de Recepcion
		TIPOOPER="Fetching"
...
if [ $COMANDO = "list" ]; then
	sftp -oPort=$PTO -b$TMP $USER@$IPORIG >$FILEDES 2>$ErrStd
	retc=$?
	if [ $retc -eq 0 ]; then
	...
	sftp -oPort=$PTO -b$TMP $USER@$IPORIG >$SalStd 2>$ErrStd
	retc=$?
fi

if [ $retc -ne 0 ]; then
	echo "$FORMATOINI [ERROR] Se presento un error:">>$LogIntelar
	echo "$FORMATOINI Codigo de retorno = $retc \n">>$LogIntelar
...
if [ $retc -ne 0 ]; then
    if [ $IP2 = "vacio" ]; then
        ...
        if [ $COMANDO = "list" ]; then
            sftp -oPort=$PTO -b$TMP $USER@$IP2 >$FILEDES 2>$ErrStd
            retc=$?
			if [ $retc -eq 0 ]; then
				echo $FORMATOINI Contenido del buzon: $BUZON>>$LogIntelar
				cat $FILEDES |grep -v sftp>>$LogIntelar 
            ...
            sftp -oPort=$PTO -b$TMP $USER@$IP2 >$SalStd 2>$ErrStd
            retc=$?
        fi
     fi
fi
