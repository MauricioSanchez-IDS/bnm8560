        ...
        : System.Windows.Forms.Form
    {
                ...
                if (ID_CEE_EMP_COB.Items.Count > CORVB.NULL_INTEGER)
                {
                    ...
                    if (CORPROC2.Obtiene_Registros() != 0)
                    {
                        ...
                        while (VBSQL.SqlNextRow(CORVAR.hgblConexion) != VBSQL.NOMOREROWS)
                        {
                            ...
                            {
                                MessageBox.Show("No se puede dar de alta la unidad porque aun no existe el Ejecutivo = 0", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                this.Cursor = Cursors.Default;
                                CORVAR.igblRetorna = VBSQL.SqlCancel(CORVAR.hgblConexion);
                                return;
                            }
        ...
        private void ID_CEE_BAJ_PB_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (ID_CEE_UNI_LB.Items.Count > 0)
                {
                    ...
                    {
                        if (MessageBox.Show("Deseas dar de baja la Unidad: " + Strings.Mid(ID_CEE_UNI_LB.Text, 19, 40).Trim(), "Tarjeta Ejecutiva", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                        {
                                ...
                                if (CORPROC2.Cuenta_Registros() <= 0)
                                {
                                    ...
                                    if (iCont == 1)
                                    {
                                        ...
                                        if (CORPROC2.Cuenta_Registros() >= 1)
                                        {
                                            ...
                                            if (iCont < 1)
                                            {
                                                MessageBox.Show("No se pudieron dar de Baja los Reportes", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                            }
                                        }
                                        ...
                                        if (CORPROC2.Cuenta_Registros() >= 1)
                                        {
                                            ...
                                            if (iCont < 1)
                                            {
                                                MessageBox.Show("No se pudieron dar de Baja las Categorías", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                            }
                                        }
                                        MessageBox.Show("Se dio de baja la Unidad: " + ID_CEE_UNI_LB.Text.Substring(0, Math.Min(ID_CEE_UNI_LB.Text.Length, Formato.sMascara(Formato.iTipo.Unidad).Length)), "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        ID_CEE_UNI_LB.Items.RemoveAt((short)ListBoxHelper.GetSelectedIndex(ID_CEE_UNI_LB));
                                    }
                                    else
                                    {
                                        MessageBox.Show("No se dio de baja la Unidad: " + ID_CEE_UNI_LB.Text.Substring(0, Math.Min(ID_CEE_UNI_LB.Text.Length, Formato.sMascara(Formato.iTipo.Unidad).Length)), "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    }
                                }
                                else
                                {
                                    MessageBox.Show("La Unidad: " + ID_CEE_UNI_LB.Text.Substring(0, Math.Min(ID_CEE_UNI_LB.Text.Length, Formato.sMascara(Formato.iTipo.Unidad).Length)) + " no se puede eliminar porque tiene Ejecutivos", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
                            }
                            else
                            {
                                MessageBox.Show("La Unidad: " + ID_CEE_UNI_LB.Text.Substring(0, Math.Min(ID_CEE_UNI_LB.Text.Length, Formato.sMascara(Formato.iTipo.Unidad).Length)) + " no se puede eliminar porque tiene Unidades Hijo", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                this.Cursor = Cursors.Default;
                                return;
                            }
                    ...
                    {
                        MessageBox.Show("No se ha seleccionado ninguna Unidad para dar de Baja", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.Cursor = Cursors.Default;
                        return;
                    }
                }
                ...
                {
                    MessageBox.Show("No hay ninguna Unidad para dar de Baja", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Cursor = Cursors.Default;
                    return;
                }
            ...
            {
                CRSGeneral.prObtenError(this.GetType().Name + "(ID_CEE_BAJ_PB_Click)", e);
            }
            finally
            {
                this.Cursor = Cursors.Default;
        ...
        private void ID_CEE_CAM_PB_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            if (ID_CEE_UNI_LB.Items.Count > CORVB.NULL_INTEGER)
            {
                ...
                {
                    this.Cursor = Cursors.Default;
                    MessageBox.Show("Aun no se ha seleccionado ninguna unidad", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
        ...
        private void ID_CEE_GRU_COB_SelectedIndexChanged(Object eventSender, EventArgs eventArgs)
        {
            ...
            {
                MessageBox.Show("No se encontro ninguna Empresa para este grupo", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Cursor = Cursors.Default;
                return;
            }
