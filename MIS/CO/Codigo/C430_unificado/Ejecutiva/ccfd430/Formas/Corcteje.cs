...
namespace TCd430
{
        ...
        : System.Windows.Forms.Form
    {
        ...
        {
            ID_CEE_GRU_COB.Enabled = abActivar;
            ID_CEE_EMP_COB.Enabled = abActivar;
            ID_CEM_MASG_3PB.Enabled = abActivar;
            ID_CEM_MASE_3PB.Enabled = abActivar;
            ID_CEE_IRE_CKB.Enabled = abActivar;
            ID_CEE_ALT_PB.Enabled = abActivar;
            ID_CEE_CAM_PB.Enabled = abActivar;
            ID_CEE_CON_PB.Enabled = abActivar;
            ID_CEE_CONS111_PB.Enabled = abActivar;
            ID_CEE_ALT_PB.Enabled = abActivar;
            cmdReenvios.Enabled = abActivar;
            cmdCancelaciones.Enabled = abActivar;
            ID_CEE_SAL_PB.Enabled = abActivar;
            ID_CEE_EMP_LB.Enabled = abActivar;
        }
        ...
        private void cmdAltasMasivas_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            try
            {
                ...
                CORVAR.pszgblsql = "select * from MTCMSV02";
                if (CORPROC2.Cuenta_Registros() > 0)
                {
                    MessageBox.Show("Existen registros en la tabla MTCMSV02 que no se han cargado", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    CORVAR.igblRetorna = VBSQL.SqlCancel(CORVAR.hgblConexion);
                    return;
                }
        ...
        private void ID_CEE_ALT_PB_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            if (ID_CEE_EMP_COB.Items.Count > CORVB.NULL_INTEGER)
            {
                ...
                {
                    frmEjecutivo.DefInstance.txtEjecutivos[1].Text = ID_CEE_EMP_COB.Text.Substring(0, 5);
        ...
        private void ID_CEE_CON_PB_Click(Object eventSender, EventArgs eventArgs)
        {
            ...
            {
                CRSGeneral.prObtenError(this.GetType().Name + "(ID_CEE_CON_PB_Click)", e);
            }
            finally
            {
                this.Cursor = Cursors.Default;
        ...
        private void ID_CEE_EMP_COB_SelectedIndexChanged(Object eventSender, EventArgs eventArgs)
        {
            ...
            try
            {
                ...
                //'*** INI:OLV NOV0304 ***
                CORVAR.pszgblsql = "SELECT A.eje_num, A.eje_nombre, ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " CASE ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = ' ' THEN 'CTA. NORMAL ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = 'O' THEN 'SOBREGIRADA ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = 'P' THEN 'PROB. COBRO ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = 'E' THEN 'CANC.C/SALD ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = 'C' THEN 'CANC.S/SALD ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = 'D' THEN 'ATRAZADA    ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " WHEN B.ths_situacion_cta = NULL THEN '- - - - - - - - - - - ' ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " END ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " FROM " + CORBD.DB_SYB_EJE + " A, " + CORBD.DB_SYB_THS + " B ";
                ...
                CORVAR.pszgblsql = CORVAR.pszgblsql + " AND   A.eje_num *= B.eje_num ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + " AND   A.emp_num = " + Conversion.Str(lEmpCve).Trim();
                ...
                if (CORPROC2.Obtiene_Registros() != 0)
                {
                    if (lTotalEjecutivos != CORVB.NULL_INTEGER)
                    {
                        ...
                        { //Or (iCont >= INT_MAX_EJECUTIVOS)
                            ...
                            CORVAR.igblEjeEstatus = VBSQL.SqlData(CORVAR.hgblConexion, 3);
                            if (CORVAR.igblEjeEstatus == "")
                            { CORVAR.igblEjeEstatus = "- - - - - - - - - - - "; }
                            //'*** FIN:OLV NOV0304 ***
                            ID_CEE_EMP_LB.Items.Add(StringsHelper.Format(CORVAR.igblEjeCve, Formato.sMascara(Formato.iTipo.Ejecutivo)) + CORCONST.SPC_TRES + CORVAR.igblEjeEstatus + CORCONST.SPC_TRES + CORVAR.szgblEjeDesc.Value.Trim());
                            iCont++;
                        };
        ...
        private void cmdCancelaciones_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (MessageBox.Show("¿Desea cancelar la cuenta del ejecutivo seleccionado?", CORSTR.STR_APP_TIT, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.No)
                {
                ...
                CORVAR.pszgblsql = CORVAR.pszgblsql + "EJ.emp_num=" + Conversion.Val(ID_CEE_EMP_COB.Text.Substring(0, Formato.sMascara(Formato.iTipo.Empresa).Length)) + " AND ";
                CORVAR.pszgblsql = CORVAR.pszgblsql + "EJ.eje_num=" + Conversion.Val(ID_CEE_EMP_LB.Text.Substring(0, Formato.sMascara(Formato.iTipo.Empresa).Length));
                ...
                {
                    MessageBox.Show("El número de cuenta no existe en el catálogo." + "\r\n" +
                            "Dé aviso del problema a su administrador de redes.", Application.ProductName);
                ...
                {
                    MessageBox.Show("No fue posible acceder al catálogo de situaciones de cuenta" + "\n\n" + cncCancelaciones.UltMsgS, Application.ProductName);
                ...
                {
                    MessageBox.Show("No fue posible crear la solicitud de cancelación de cuenta, reintente la operación más tarde." +
                            "\r\n" + "Si el problema persiste comuníquese con su administrador de sistemas." + "\r\n" +
                            "\r\n" + cncCancelaciones.UltMsgS, Application.ProductName);
                ...
                frmCancLog.DefInstance.prLog(cncCancelaciones.TransLogS);
                MessageBox.Show("La solicitud de baja de la cuenta " + cncCancelaciones.CuentaS + " se creó con éxito." + "\r\n" +
            ...
            catch (Exception ex)
            {
                MessageBox.Show("Ocurrió el siguiente error en el proceso:" + "\r\n" + "Número: " + Information.Err().Number + "\r\n" +
                        ...
                        "Módulo: CORCTEJE::cmdCancelaciones::Click()", Application.ProductName);
                cncCancelaciones = null;
                mprActivarBotones(true);
                this.Cursor = Cursors.Default;
