        ...
        : System.Windows.Forms.Form
    {
        ...
        */
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else if (CORMNEJE.DefInstance.Tag.ToString() == CORCONST.TAG_CONSULTA)
                        {
                            ...
                            _ID_MEM_ETT_TXT_5.Enabled = false;
        ...
        private void Muestra_Datos()
        {
            ...
            string mStrEjecutivoBnmx = String.Empty;
            string mStrNoSucursalPromotora = String.Empty;
            ...
            if (CORPROC2.Obtiene_Registros() != 0)
            {
                if (VBSQL.SqlNextRow(CORVAR.hgblConexion) == VBSQL.MOREROWS)
                {
                    ...
                    if (this.Tag.ToString() == CORCONST.TAG_ALTA)
                    {
                        ...
                        CRSGeneral.iEjeBnx = Convert.ToInt32(Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion, 13)));
                        if (CRSGeneral.iEjeBnx != 0)
                        { FnBoolBuscaComboEjeBnx(CRSGeneral.iEjeBnx); }
                        else
                        { CboEjecutivoBnx.SelectedIndex = -1; }
            ...
            if (CORMNEJE.DefInstance.Tag.ToString() == CORCONST.TAG_CAMBIO || CORMNEJE.DefInstance.Tag.ToString() == CORCONST.TAG_CONSULTA || CORMNEJE.DefInstance.Tag.ToString() == "CAMBIO LIMITE DE CREDITO")
            {
                ...
                if (CORPROC2.Obtiene_Registros() != 0)
                {
                    ...
                    if (VBSQL.SqlNextRow(CORVAR.hgblConexion) == VBSQL.MOREROWS)
                    {
                        ...
                        mStrNoSucursalPromotora = VBSQL.SqlData(CORVAR.hgblConexion, 29);
                        mStrNoSucursalPromotora = mStrNoSucursalPromotora.Trim();

                        mStrEjecutivoBnmx = VBSQL.SqlData(CORVAR.hgblConexion, 55);
                        ...
                        if ((mStrEjecutivoBnmx != "") && (mStrEjecutivoBnmx != "0"))
                        {
                            if (!FnBoolBuscaComboEjeBnx(int.Parse(mStrEjecutivoBnmx)))
                            {
                                CboEjecutivoBnx.Items.Add(new ListBoxItem(mStrEjecutivoBnmx + " EJECUTIVO NO EN CATALOGO", int.Parse(mStrEjecutivoBnmx)));
                                CboEjecutivoBnx.SelectedIndex = 0;
                            }
                        }
                        else
                        {
                            //'El valor del ejecutivo no esta capturado
                            CboEjecutivoBnx.SelectedIndex = -1;
                            ColSucursal = new Collection();
                        }

                        if (mStrNoSucursalPromotora == "")
                        {
                            CboSucursal.Items.Add(new ListBoxItem(mStrNoSucursalPromotora + " SUCURSAL NO EN CATALAGO", 0));
                            CboSucursal.SelectedIndex = 0;
                        }
                        else
                            if (!FnBoolBuscaComboSucursal(int.Parse(mStrNoSucursalPromotora)))
                            {
                                CboSucursal.Items.Add(new ListBoxItem(mStrNoSucursalPromotora + " SUCURSAL NO EN CATALAGO", int.Parse(mStrNoSucursalPromotora)));
                                CboSucursal.SelectedIndex = 0;
                            }
        ...
        private void Realiza_cambio()
        {
            ...
            N_ActualizaS111.ClsConectaS111 ObjConexionS111 = null;
            //ObjConexionS111
            ...
            try
            {
                ...
                string pszCampo = Valida_Campos(1);
                ...
                int lEmpNum = Convert.ToInt32(Conversion.Val(ID_MEE_CTA_TXT.Text)); //emp_num
                int iEjeRoll = 9; //eje_roll_over
                string pszEjeNom = ID_MEE_NEJ_EB.Text.Trim(); //eje_nom

                string pszEjeRFC = Seguridad.fncsSustituir(ID_MEE_RFC_EB.CtlText.Trim(), "-", ""); //RFC

                //lNumNom = Val(ID_MEE_NOM_PIC.Text) comenta yuria                                 'eje_num_nom
                string sNumNom = ID_MEE_NOM_PIC.CtlText; //agrega yuria 12/10/06 para que número de nómina sea alfanumérico //eje_num_nom
                int lEjeSueldo = Convert.ToInt32(Conversion.Val(ID_MEE_SDO_EB.CtlText)); //eje_sueldo
                ...
                {
                    VB6.SetItemString(CORCTEJE.DefInstance.ID_CEE_EMP_LB, ListBoxHelper.GetSelectedIndex(CORCTEJE.DefInstance.ID_CEE_EMP_LB), Strings.Mid(VB6.GetItemString(CORCTEJE.DefInstance.ID_CEE_EMP_LB, ListBoxHelper.GetSelectedIndex(CORCTEJE.DefInstance.ID_CEE_EMP_LB)), 1, 6) + ID_MEE_NEJ_EB.Text.Trim());
                }
                ...
                if (ID_MEE_NEJ_EB.Text.Trim() != CORVB.NULL_STRING)
                {
                    ...
                    else
                    {
                        ...
                        if (ValidaCambioS111())
                        {
                            ...
                            ////AIS-1146 NGONZALEZ
                            //ObjConexionS111.set_PrCheker(ref mdlParametros.bgCamLimCred);
                            ////AIS-1146 NGONZALEZ
                            ...
                            //        else
                            //        {
                            //            MdlCambioMasivo.MsgError(er.Message);
                            //            ObjConexionS111 = null;
                            //            return;
                            //        }
                            ...
                            { //Cambio de limite de credito
                                if (mdlParametros.bgCamLimCred)
                                {
                                    if (EnviaCambioS111("Credito", ObjConexionS111))
                                    {
                                        ...
                                        MessageBox.Show("No se actualizó el Tope del tarjetahabiente en el S111", CORSTR.STR_APP_TIT, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                        //End If
                                    ...
                                    else
                                    {
                                        if (mbPrimTran)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (Actualiza_Campos_M111(ref tempRefParam4))
                                                {
                                                    ...
                                                    {
                                                        this.Cursor = Cursors.Default;
                                                        MessageBox.Show("No se actualizó el Tope del tarjetahabiente en el M111", CORSTR.STR_APP_TIT, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                                        string tempRefParam3 = "";
                                                        mdlSeguridad.fncFirmaS041B(ref tempRefParam3, "", "", mdlSeguridad.enuTipoDlgSeguridad.tDesfirmaS041);
                                        ...
                                        else
                                        {
                                            ...
                                            if ((CORVAR.glstrLimCred.IndexOf("linea", StringComparison.CurrentCultureIgnoreCase) + 1) == 0)
                                            {
                                                ...
                                                //ais-1608 chidalgo
                                                int index = ID_MEE_LIM_FTB.CtlText.IndexOf('.');
                                                if (index == -1)
                                                {
                                                    index = ID_MEE_LIM_FTB.CtlText.Length;
                                                }
                                                llValPost = Int32.Parse(ID_MEE_LIM_FTB.CtlText.Substring(0, index).Replace("$", "").Replace(",", ""));
                                                mdlSeguridad.llNuevoCredito = llValPost;
                        ...
                        if (msCamDatAdi == "DatosAdi" || blUpdate)
                        {
                            ...
                            CORVAR.pszgblsql = CORVAR.pszgblsql + "' ',"; //FSWB NR 20070221
                            CORVAR.pszgblsql = CORVAR.pszgblsql + "0"; //FSWB NR 20070221
        ...
        private string Valida_Campos(int IntAccion)
        {
            ...
            try
            {
                ...
                else if ((CboEjecutivoBnx.SelectedIndex == -1) && (IntAccion == ALTA))
                {
                    pszValida = "Ejecutivo que realiza la venta";
                    Fichero(3);
                    CboEjecutivoBnx.Focus();
                }
                else if ((ObjSucursal == null) || (CboSucursal.SelectedIndex == -1))
                {
                    if (IntAccion == ALTA)
                    {
                        pszValida = "Sucursal Promotora";
                        Fichero(3);
                        CboSucursal.Focus();
                    }
                    else
                ...
                else if ((ObjSucursal.Status == "N") && (IntAccion == ALTA))
                {
                    pszValida = "Sucursal Inactiva";
                    Fichero(3);
                    CboSucursal.Focus();
                }
        ...
        /*
                private void Alta_Ejecutivo()
                {
                    ...
                    try
                    {
                        ...
                        string pszCampo = Valida_Campos(0);
                        ...
                        //CORVAR.pszgblsql = "select emp_tipo_envio, emp_cred_tot, emp_acum_cred from MTCEMP01 where emp_num = " & str$(lEmpNum) & " and eje_prefijo=" & Format$(igblPref) & " and gpo_banco=" & Format$(igblBanco)
                        // Modif. 18/Jun/2007 Calc Lim Real
                        ...
                        iSucProm = ObjSucursal.IdSucursal;                                   //'sucursal promotora
                        CRSGeneral.iEjeBnx = VB6.GetItemData(CboEjecutivoBnx, CboEjecutivoBnx.SelectedIndex); //'Ejecutivo que realiza la venta
        ...
        private void BtnAgregarSuc_Click(object sender, EventArgs e)
        {
            ...
            lngEjeBnx = CboEjecutivoBnx.SelectedIndex;
            if (lngEjeBnx > -1)
            {
                CORMNEJB.DefInstance.Tag = CORCONST.TAG_CAMBIO;
                CORVAR.lgblEjxCve = VB6.GetItemData(CboEjecutivoBnx, lngEjeBnx);

                CORMNEJB.DefInstance.ShowDialog();
                CboEjecutivoBnx_SelectedIndexChanged(CboEjecutivoBnx, new EventArgs());
            }
            else
            {
        ...
        private void CboEjecutivoBnx_SelectedIndexChanged(object sender, EventArgs e)
        {
            int lngIndex;
            lngIndex = CboEjecutivoBnx.SelectedIndex;
            ObjSucursal = null;
            CboSucursal.Items.Clear();
            if (lngIndex > -1)
            {
                if (!FnBolLeeSucursal(VB6.GetItemData(CboEjecutivoBnx, lngIndex)))
                {
        ...
        private void CboSucursal_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ObjSucursal = FnAsignaSucursal("K_" + VB6.GetItemData(CboSucursal, CboSucursal.SelectedIndex), CboSucursal.SelectedIndex);
                if (ObjSucursal != null)
                {
                    txtEstatusSucursal.Text = ObjSucursal.Status;
                }
                else
                {
                    txtEstatusSucursal.Text = "N";
                }
            }
            else
            {
                txtEstatusSucursal.Text = "";
            }
        }

        private void BtnBuscaNomina_Click(object sender, EventArgs e)
        {
            int iResp = 0; //La respuesta a la advertencia

            if (MskNomina.ClipText.Trim() != "")
            {
                if (!FnBoolBuscaComboEjeBnx(int.Parse(MskNomina.ClipText)))
                {
                    this.Cursor = Cursors.Default;
                    ...
                    if (iResp == CORVB.IDYES)
                    {
                        ...
                        CORMNEJB.DefInstance.ShowDialog();
                        FnLeeEjeBnmx();
                        if (CboEjecutivoBnx.Items.Count > 0)
                        {
                            CboEjecutivoBnx.SelectedIndex = 0;
                        }
                    }
            ...
            if (lngEjeBnx > -1)
            {
                ...
                if (CORPROC2.Obtiene_Registros() != 0)
                {
                    ...
                    while (VBSQL.SqlNextRow(CORVAR.hgblConexion) != VBSQL.NOMOREROWS)
                    {
                        ...
                        CboSucursal.Items.Add(new ListBoxItem(StringsHelper.Format(ObjSucursal.IdSucursal, "@@@@") + " " + ObjSucursal.Descripcion, ObjSucursal.IdSucursal));
        ...
        private bool FnLeeEjeBnmx()
        {
            try
            {
                ...
                CboEjecutivoBnx.Items.Clear();
                if (CORPROC2.Obtiene_Registros() != 0)
                {
                    while (VBSQL.SqlNextRow(CORVAR.hgblConexion) != VBSQL.NOMOREROWS)
                    {
                        CboEjecutivoBnx.Items.Add(new ListBoxItem(StringsHelper.Format(int.Parse(VBSQL.SqlData(CORVAR.hgblConexion, 1)), "000000") + " " + VBSQL.SqlData(CORVAR.hgblConexion, 2), int.Parse(VBSQL.SqlData(CORVAR.hgblConexion, 1))));
                    }
                }
                return true;
            }
        ...
        private bool FnBoolBuscaComboEjeBnx(int lngEjeBnx)
        {
            ...
            lngEjecutivos = FnLngBuscaElementoCombo(CboEjecutivoBnx, lngEjeBnx);
            CboEjecutivoBnx.SelectedIndex = lngEjecutivos;
            CboEjecutivoBnx_SelectedIndexChanged(CboEjecutivoBnx, new EventArgs());
            if (lngEjecutivos > -1)
            { return true; }
            else
        ...
        {
            int mlngSucursal;
            mlngSucursal = FnLngBuscaElementoCombo(CboSucursal, lngSucursal);
            CboSucursal.SelectedIndex = mlngSucursal;
            CboSucursal_SelectedIndexChanged(CboSucursal, new EventArgs());
            if (mlngSucursal > -1)
            { return true; }
            else
            { return false; }
        ...
        private void ID_MEE_LIM_FTB_Change(object sender, EventArgs e)
        {

        }

        private void MskNomina_Change(object sender, EventArgs e)
        {

        }

        private void ID_MEE_NEM_EB_TextChanged(object sender, EventArgs e)
        {
