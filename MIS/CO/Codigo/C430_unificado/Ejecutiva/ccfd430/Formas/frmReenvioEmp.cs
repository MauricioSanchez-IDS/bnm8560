		...
		: System.Windows.Forms.Form
		{
			...
			private void  cmdConsIndividual_Click( Object eventSender,  EventArgs eventArgs)
			{
					try
					{
                            ...
                            vspmEmpresas.Row = 1;
                            vspmEmpresas.Col = 1;
                            //vspmEmpresas.Action = 0;
                            vspmEmpresas.Clear();
                            FarPoint.Win.Spread.CellClickEventArgs e1 = new FarPoint.Win.Spread.CellClickEventArgs(new FarPoint.Win.Spread.SpreadView(fpsvspmEmpresas), 1, 1, 0, 0, MouseButtons.Left, false, false);
                            object sender1 = null;
                            vspmEmpresas_Click(sender1, e1);
            ...
            private void ConsultaMTCIND(Softtek.Util.Win.Spread.SpreadWrapper  vsplGrid, colReenvioEmp colReenvioEmp)
			{
					...
					try
					{
							...
							//vsplGrid.Action = (FPSpreadADO.ActionConstants)3;
                            vsplGrid.Clear();
							...
							{
                                vsplGrid.set_ColWidth(iCont, int.Parse(vsplGrid.get_MaxTextColWidth(iCont).ToString()) + 50);
							}
							this.Cursor = Cursors.Default;
			...
			private bool fncReenvioEjecutivosB( int iIndxItem,  colReenvioEmp colReenvioEmp)
			{
                    ...
                    lAltaEmp.pszSucTransmisora = colReenvioEmp[iIndxItem.ToString().Trim()].pszSucTransmisora;
                    lAltaEmp.pszSucPromotora = colReenvioEmp[iIndxItem.ToString().Trim()].pszSucPromotora;
					...
					dlgAlta.sSexo = "F";
                    dlgAlta.iSucTransmisora = Convert.ToInt32(Conversion.Val(colReenvioEmp[iIndxItem.ToString().Trim()].pszSucTransmisora));
                    dlgAlta.iSucPromotora = Convert.ToInt32(Conversion.Val(colReenvioEmp[iIndxItem.ToString().Trim()].pszSucPromotora));
                    ...
                    dlgAlta.sCuenta = StringsHelper.Format(lcteCliente.ProductoPRD.PrefijoL, "0000") + StringsHelper.Format(lcteCliente.ProductoPRD.BancoL, "00") + StringsHelper.Format(lAltaEmp.illNumEmpresa, Formato.sMascara(Formato.iTipo.Empresa)) + StringsHelper.Format(0, Formato.sMascara(Formato.iTipo.Ejecutivo)) + "9";
                    dlgAlta.sCuenta = dlgAlta.sCuenta + CORPROC.Calcula_digito_verif(ref dlgAlta.sCuenta);
            ...
            private void vspmEmpresas_Click(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
            {
                object pvStatus;
                ...
                if (e != null && e.Row > 0)
                {
                    if (vspmEmpresas.GetText(5, e.Row, ref pvStatus))
                    {
                        if (pvStatus.ToString() == "")
                        {
                            pvStatus = "0";
                        }
                        CRSGeneral.prSemaforoReenvios(ref shpS016, ref shpS111, ref shpC430, ref shpComplementaria, CRSGeneral.enmSemaforoModulos.SMD_EMPRESAS, Convert.ToInt32(pvStatus));
                    }
                    else
                    {
                        CRSGeneral.prSemaforoReenvios(ref shpS016, ref shpS111, ref shpC430, ref shpComplementaria, CRSGeneral.enmSemaforoModulos.SMD_EMPRESAS, -1);
                    }
                }
            }

            private void vspmEmpresas_LeaveCell(object sender, FarPoint.Win.Spread.LeaveCellEventArgs e)
            {
                FarPoint.Win.Spread.CellClickEventArgs e1;
                e1 = null;
                vspmEmpresas_Click(sender, e1);
            }
