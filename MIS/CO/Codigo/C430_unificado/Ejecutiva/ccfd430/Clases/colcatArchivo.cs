...
namespace TCd430
{
	...
	public bool fncValidaInformacionB( string cadenaPathFile)
	{
			...
			try
			{
                    ...
                    FileSystem.FileOpen(mdlParametros.llNumFileErr, Path.GetDirectoryName(Environment.SpecialFolder.ApplicationData.ToString()) + "\\Archivo_Errores.txt", OpenMode.Output, OpenAccess.Default, OpenShare.Default, -1);
						...
						if (Strings.Mid(mdlParametros.slCadenaValidar, 1, 6) == "HEADER")
						{
							...
							if (blResHeader)
							{
								...
								if (CORPROC2.Obtiene_Registros() == (-1))
								{
									...
									{
										MessageBox.Show("No se pueden dar altas masivas el monto del credito reportado en el archivo es mayor al de la Empresa", "Tarjeta Ejecutiva", MessageBoxButtons.OK, MessageBoxIcon.Error);
	...
	public bool fncGrabaTablaMTCMSV01B()
	{
			...
			try
			{
					...
					if (mdlParametros.gcarcAltasMasivas.Count != 0)
					{
						...
						{
							CORVAR.pszgblsql = "insert into MTCMSV02 values(" + mdlParametros.gprdProducto.PrefijoL.ToString() + " , " + mdlParametros.gprdProducto.BancoL.ToString() + " , ";
                ...
                catch (Exception e)
			{
				...
				CRSGeneral.prObtenError(this.GetType().Name + "(ValidaInformacion, Insercion Tabla MTCMSV02)",e );
				result = false;
				mdlParametros.prQuitaObjetos();
				FileSystem.FileClose(mdlParametros.llNumFile);
				FileSystem.FileClose(mdlParametros.llNumFileErr);
