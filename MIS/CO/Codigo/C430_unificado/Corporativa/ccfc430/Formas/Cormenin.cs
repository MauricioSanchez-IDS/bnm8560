	...
	internal partial class CORMENIN
		: System.Windows.Forms.Form
		{
			...
			private void  Muestra_Datos()
			{
					...
					CORVAR.pszgblsql = CORVAR.pszgblsql + " and pro_nomarch like ('E%')";
					...
					if (CORPROC2.Obtiene_Registros() != 0)
					{
						...
						while(VBSQL.SqlNextRow(CORVAR.hgblConexion) != VBSQL.NOMOREROWS)
						{
							...
							{ //Sin error
								pszAccion = CORVB.NULL_STRING;
								pszSituacion = "OK";
							} else if (iEstatus == 2) {  //Problemas con el espacio del disco
								pszAccion = "Problemas de espacio, hablar a Redes.";
								pszSituacion = "ERROR";
							} else if (iEstatus == 1 || iEstatus >= 4 && iEstatus <= 19) {  //Aviso a ingeniería
								pszAccion = "Avisar a Ingeniería.";
								pszSituacion = "ERROR";
							} else if (iEstatus >= 20 && iEstatus <= 29) {  //Realizar reenvío
							...
							//THS
							if ((pszNomArch.IndexOf("THS") + 1) > CORVB.NULL_INTEGER)
							{
								if (Strings.Mid(pszNomArch, pszNomArch.IndexOf("THS") + 1, 3) == "THS")
								{
							...
							//HIH
							if ((pszNomArch.IndexOf("HIH") + 1) > CORVB.NULL_INTEGER)
							{
								if (Strings.Mid(pszNomArch, pszNomArch.IndexOf("HIH") + 1, 3) == "HIH")
								{
									pszCadena = StringsHelper.MidAssignment(pszCadena, 1, pszNomArch);
							...
							//HIS
							if ((pszNomArch.IndexOf("HIS") + 1) > CORVB.NULL_INTEGER)
							{
								if (Strings.Mid(pszNomArch, pszNomArch.IndexOf("HIS") + 1, 3) == "HIS")
								{
									pszCadena = StringsHelper.MidAssignment(pszCadena, 1, pszNomArch);
							...
							//PLA
							if ((pszNomArch.IndexOf("PLA") + 1) > CORVB.NULL_INTEGER)
							{
								if (Strings.Mid(pszNomArch, pszNomArch.IndexOf("PLA") + 1, 3) == "PLA")
								{
									pszCadena = StringsHelper.MidAssignment(pszCadena, 1, pszNomArch);
