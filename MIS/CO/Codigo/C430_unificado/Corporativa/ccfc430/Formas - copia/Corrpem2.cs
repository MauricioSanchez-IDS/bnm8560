        ...
        : System.Windows.Forms.Form
    {
        ...
        private int Carga_Reporte()
        {
            ...
            try
            {
                ...
                CORVAR.pszgblsql = "exec reporte_afi_emp " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString();
                ...
                { //Ocurrio algun error en la ejecución SQL
                    ...
                    { //Obtenemos los Registros regresados por el Select
                        ...
                        lNumTarjVenc2 = CORVB.NULL_INTEGER;
                        lNumTarjVenc3 = CORVB.NULL_INTEGER;
                        if (dSaldosVencidos > 0)
                        {
                            //SALDOS CON UN MES VENCIDO
                            CORVAR.pszgblSql2 = "exec reporte_saldos_venc1 " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + lNumEmpresa.ToString();
                            if (CORPROC2.Consulta_Registros() != 0)
                            {
                                if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                                {
                                    lNumTarjVenc1 = Convert.ToInt32(Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1)));
                                    dSaldoVenc1mes = Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 2));
                                }
                            }
                            else
                            {
                            ...
                            //SALDOS CON UN 2 MESES
                            //        pszgblSql2 = "exec reporte_saldos_venc2 " + Format$(lNumEmpresa)
                            CORVAR.pszgblSql2 = "exec reporte_saldos_venc222 " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + lNumEmpresa.ToString();
                            if (CORPROC2.Consulta_Registros() != 0)
                            {
                                if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                                {
                                    lNumTarjVenc2 = Convert.ToInt32(Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1)));
                                    dSaldoVenc2mes = Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 2));
                                }
                            }
                            else
                            {
                            ...
                            lRetorna = VBSQL.SqlCancel(CORVAR.hgblConexion2); //Realiza fin de Query                'Realiza fin de Query

                            //SALDOS CON UN 3 MESES O MAS VENCIDOS
                            CORVAR.pszgblSql2 = "exec reporte_saldos_venc3 " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + lNumEmpresa.ToString();
                            if (CORPROC2.Consulta_Registros() != 0)
                            {
                                if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                                {
                                    lNumTarjVenc3 = Convert.ToInt32(Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1)));
                                    dSaldoVenc3mes = Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 2));
                                }
                            }
                            else
                            {
                        ...
                        //      lTotTarjVenc1 = lTotTarjVenc1 + lNumTarjVenc1
                        //      lTotTarjVenc2 = lTotTarjVenc2 + lNumTarjVenc2
                        //      lTotTarjVenc3 = lTotTarjVenc3 + lNumTarjVenc3
                        //      dTotVenc1mes = dTotVenc1mes + dSaldoVenc1mes
                        //      dTotVenc2mes = dTotVenc2mes + dSaldoVenc2mes
                        //      dTotVenc3mes = dTotVenc3mes + dSaldoVenc3mes

                        //Obtiene numero de Compras y su Importe
                        //***** INICIO CODIGO ANTERIOR FSWBMX *****
                        //pszgblSql2 = "exec reporte_dej_compras " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO ANTERIOR FSWBMX *****
                        //12.11.2001 Código Anterior
                        //***** INICIO CODIGO NUEVO FSWBMX *****
                        //      pszgblSql2 = "exec reporte_dej_compras " + Format$(igblPref) + "," + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO NUEVO FSWBMX *****
                        //12.11.2001 Fin de Código Anterior
                        //EISSA 09.11.2001 Código Nuevo Cambio de parámetros de los Sp's, ahora sólo se requiere utilizar el mes actual
                        CORVAR.pszgblSql2 = "exec reporte_dej_compras " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString() + "," + lNumEmpresa.ToString();
                        ...
                        //Obtiene el numero de Disposisciones y su importe
                        //***** INICIO CODIGO ANTERIOR FSWBMX *****
                        //pszgblSql2 = "exec reporte_dej_disposiciones " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO ANTERIOR FSWBMX *****
                        //12.11.2001 Inicio Código Anterior
                        //***** INICIO CODIGO NUEVO FSWBMX *****
                        //      pszgblSql2 = "exec reporte_dej_disposiciones " + Format$(igblPref) + "," + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO NUEVO FSWBMX *****
                        //12.11.2001 Fin Código Anterior

                        //EISSA 09.11.2001 Código Nuevo Cambio de parámetros de los Sp's, ahora sólo se requiere utilizar el mes actual
                        CORVAR.pszgblSql2 = "exec reporte_dej_disposiciones " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString() + "," + lNumEmpresa.ToString();
                        //EISSA 09.11.2001 Fin de Código Nuevo
                        ...
                        if (CORPROC2.Consulta_Registros() != 0)
                        {
                            if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                            {
                                dDispEfvo = Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1));
                        ...
                        //Obtiene el importe por Comisiones
                        //***** INICIO CODIGO ANTERIOR FSWBMX *****
                        //pszgblSql2 = "exec reporte_por_comisiones " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO ANTERIOR FSWBMX *****
                        //12.11.2001 Inicio Código Anterior
                        //***** INICIO CODIGO NUEVO FSWBMX *****
                        //      pszgblSql2 = "exec reporte_por_comisiones " + Format$(igblPref) + "," + Format$(igblBanco) + "," + ifncMes(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** FIN DE CODIGO NUEVO FSWBMX *****
                        //12.11.2001 Fin Código Anterior
                        //EISSA 09.11.2001 Código Nuevo Cambio de parámetros de los Sp's, ahora sólo se requiere utilizar el mes actual
                        CORVAR.pszgblSql2 = "exec reporte_por_comisiones " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString() + "," + lNumEmpresa.ToString();
                        //EISSA 09.11.2001 Fin de Código Nuevo
                        ...
                        if (CORPROC2.Consulta_Registros() != 0)
                        {
                            if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                            {
                                dComisiones = Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1));
                            }
                        }
                        else
                        {
                        ...
                        //ros      dTotFactura = dTotFactura + dFacturacion

                        //dPromPagare = dFacturacion / lTransacciones
                        //dTotPromPag = dTotPromPag + dPromPagare

                        //Obtiene_Status lNumEmpresa, lCtasActivas, lCtasCongeladas, hConexion

                        //Obtiene el total de cuentas activas, todas aquellas que tengan:
                        // -Disposiciones
                        // -Comisiones
                        // -Compras
                        // -Cualquier transaccion de cargo

                        //ros      pszgblSql2 = "exec reporte_ctas_activas " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA) + "," + Format$(lNumEmpresa)
                        //***** INICIO CODIGO ANTERIOR FSWBMX *****
                        ...
                        if (CORPROC2.Consulta_Registros() != 0)
                        {
                            if (VBSQL.SqlNextRow(CORVAR.hgblConexion2) == VBSQL.MOREROWS)
                            {
                                lCtasActivas = Convert.ToInt32(Conversion.Val(VBSQL.SqlData(CORVAR.hgblConexion2, 1)));
                            }
                        }
                        else
                        {
                        ...
                        //ros     dTotPromTar = dTotPromTar + dPromTarjeta
                        //ros     lTotCtaActi = lTotCtaActi + lCtasActivas

                        //ros      lTotNumEmp = lTotNumEmp + 1
        ...
        private void Coloca_totales()
        {
            ...
            //Empresas en Total
            ID_EMA_REP_SS.Col = 2;
            ID_EMA_REP_SS.TypeHAlign = FarPoint.Win.Spread.CellHorizontalAlignment.Left;
            ...
            ID_EMA_REP_SS.Col = 5;
            ID_EMA_REP_SS.CellType = new FarPoint.Win.Spread.CellType.TextCellType();
        ...
        private void Formatea_Spread()
        {
            ...
            ID_EMA_REP_SS.Col = 1;
            ID_EMA_REP_SS.set_ColWidth(1, 7 * 10);
            ID_EMA_REP_SS.Text = "Número";

            ID_EMA_REP_SS.Col = 2;
            ID_EMA_REP_SS.set_ColWidth(2, 35 * 8);
            ID_EMA_REP_SS.Text = "Empresa";

            ID_EMA_REP_SS.Col = 3;
            ID_EMA_REP_SS.set_ColWidth(3, 10 * 5);
            ID_EMA_REP_SS.Text = "Tarjetas";


            ID_EMA_REP_SS.Col = 4;
            ID_EMA_REP_SS.set_ColWidth(4, 10 * 11);
            ID_EMA_REP_SS.Text = "Ctas Activas";

            ID_EMA_REP_SS.Col = 5;
            ID_EMA_REP_SS.set_ColWidth(5, 18 * 6);
            ID_EMA_REP_SS.Text = "Facturación";


            ID_EMA_REP_SS.Col = 6;
            ID_EMA_REP_SS.set_ColWidth(6, 13 * 9);
            ID_EMA_REP_SS.Text = "Prom. Tarjs.";

            ID_EMA_REP_SS.Col = 7;
            ID_EMA_REP_SS.set_ColWidth(7, 10 * 11);
            ID_EMA_REP_SS.Text = "# Tarjs. 1 M.V.";


            ID_EMA_REP_SS.Col = 8;
            ID_EMA_REP_SS.set_ColWidth(8, 18 * 6);
            ID_EMA_REP_SS.Text = "Sdos 1 M.V.";

            ID_EMA_REP_SS.Col = 9;
            ID_EMA_REP_SS.set_ColWidth(9, 10 * 11);
            ID_EMA_REP_SS.Text = "# Tarjs. 2 M.V.";


            ID_EMA_REP_SS.Col = 10;
            ID_EMA_REP_SS.set_ColWidth(10, 18 * 6);
            ID_EMA_REP_SS.Text = "Sdos 2 M.V.";

            ID_EMA_REP_SS.Col = 11;
            ID_EMA_REP_SS.set_ColWidth(11, 10 * 11);
            ID_EMA_REP_SS.Text = "# Tarjs. >= 3 M.V.";


            ID_EMA_REP_SS.Col = 12;
            ID_EMA_REP_SS.set_ColWidth(12, 18 * 6);
            ID_EMA_REP_SS.Text = "Sdos >= 3 M.V.";


            ID_EMA_REP_SS.Col = 13;
            ID_EMA_REP_SS.set_ColWidth(13, 18 * 6);
        ...
        private void Llena_Concentrado()
        {
            ...
            CORVAR.pszgblsql = "exec reporte_afi_saldos " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString();
            ...
            if (CORPROC2.Obtiene_Registros() != 0)
            {
                ...
                //***** INICIO CODIGO ANTERIOR FSWBMX *****
                //pszgblsql = "exec reporte_afi_compras " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA)
                //***** FIN DE CODIGO ANTERIOR FSWBMX *****
                //12.11.2001 Inicio Código Anterior
                //***** INICIO CODIGO NUEVO FSWBMX *****
                //    pszgblsql = "exec reporte_afi_compras " + Format$(igblPref) + "," + Format$(igblBanco) + "," + Format$(igblMesDiaDEA)
                //***** FIN DE CODIGO NUEVO FSWBMX *****
                //12.11.2001 Inicio Código Anterior
                //EISSA 12.11.2001 Código Nuevo Cambio de parámetros de los Sp's, ahora sólo se requiere utilizar el mes actual
                CORVAR.pszgblsql = "exec reporte_afi_compras " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString();
                //EISSA 12.11.2001 Fin de Código Nuevo
                ...
                if (CORPROC2.Obtiene_Registros() != 0)
                {
                    ...
                    //***** INICIO CODIGO ANTERIOR FSWBMX *****
                    //pszgblsql = "exec reporte_afi_disposiciones " + Format$(igblBanco) + "," + Format$(igblMesDiaDEA)
                    //***** FIN DE CODIGO ANTERIOR FSWBMX *****
                    //12.11.2001 Inicio Código Anterior
                    //***** INICIO CODIGO NUEVO FSWBMX *****
                    //      pszgblsql = "exec reporte_afi_disposiciones " + Format$(igblPref) + "," + Format$(igblBanco) + "," + Format$(igblMesDiaDEA)
                    //***** FIN DE CODIGO NUEVO FSWBMX *****
                    //12.11.2001 Fin Código Anterior
                    //EISSA 12.11.2001 Código Nuevo Cambio de parámetros de los Sp's, ahora sólo se requiere utilizar el mes actual
                    CORVAR.pszgblsql = "exec reporte_afi_disposiciones " + CORVAR.igblPref.ToString() + "," + CORVAR.igblBanco.ToString() + "," + Formato.ifncMes(CORVAR.igblMesDiaDEA).ToString();
                    //EISSA 12.11.2001 Fin de Código Nuevo
                    if (CORPROC2.Obtiene_Registros() != 0)
                    {
                        if (VBSQL.SqlNextRow(CORVAR.hgblConexion) == VBSQL.MOREROWS)
                    ...
                    else
                    {
                        CORVAR.igblRetorna = VBSQL.SqlCancel(CORVAR.hgblConexion); //Terminamos el SQL
                        //AIS-1182 NGONZALEZ
                        CORVAR.igblErr = API.USER.PostMessage(Handle.ToInt32(), CORVB.WM_CLOSE, CORVB.NULL_INTEGER, CORVB.NULL_INTEGER);
                    }
                }
                else
                {
            ...
            ID_DAT_RES_TXT[0].Text = StringsHelper.Format(Math.Truncate(dSaldoAnt), "$##,###,###,##0.00");
            ...
            ID_DAT_RES_TXT[7].Text = StringsHelper.Format(Math.Truncate(dSaldoAnt) + dCompras + dDisposiciones - dPagosyAbonos + dComisiones + dIntereses + dIva, "$##,###,###,##0.00");
