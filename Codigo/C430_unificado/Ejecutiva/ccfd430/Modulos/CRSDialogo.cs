...
namespace TCd430
{
	class CRSDialogo
	{
        ...
        public const string cteIdentificacion = "101599";//Se solicita el cambio del Id 100299 => 101599  17/01/18
		...
		public struct mtcEjecutivo
		{
			...
			[MarshalAs(System.Runtime.InteropServices.UnmanagedType.ByValTStr,SizeConst=4)] public string sEjeEstado;
			public int iEjeZP;
			public int lEjeCP;
			...
			[MarshalAs(System.Runtime.InteropServices.UnmanagedType.ByValTStr,SizeConst=16)] public string sEjeCuentaPadre;
		...
		*/
		...
		//
		//
		//Public Function ilLayOutAltaS111S016(spArchivo As String, ipTipoAlta As iTipo) As Boolean
		//Dim slRegistro          As String
		//Dim ilRegistro          As Integer
		//Dim ilArchivoDisponible As Integer
		//Dim dlgDialogo()        As DatosDialogo
		//Dim slDialogo           As String
		//Dim cnxConexion         As New Conexion
		//Dim blConexion          As Boolean
		//Dim ejelEjecutivo       As DatosEjecutivo
		//Dim mtclEjecutivo       As mtcEjecutivo
		//Dim ilAltaStatus        As Integer
		//Dim spSeparador         As String
		//Dim slEnviarut          As String
		//On Error GoTo ErrLayOutAltaS111S016
		//    Screen.MousePointer = vbHourglass
		//'Validaciones
		//    'Obtiene las longitudes de los distintos productos y los aloja en sg
		//    prLongitudes
		//    spSeparador = Chr(9)
		//
		//    'Investiga el archivo que esta disponible
		//    ilArchivoDisponible = FreeFile
		//
		//    'Valida que exista el CommDriver
		//    If Dir$(pszgblPathComDrive) = Empty Then
		//        blConexion = False
		//        MsgBox "No se ha logrado establecer la conexión al S111" & vbCrLf & "Consulte la Conexión", vbCritical, "Conexión al S111"
		//        Screen.MousePointer = vbDefault
		//        Exit Function
		//    End If
		//
		//    'Inicia la conexión al S111
		//    If cnxConexion.Inicia_Conexion = True Then
		//        blConexion = True
		//    Else
		//      blConexion = False
		//        MsgBox "No se ha logrado establecer la conexión al S111" & vbCrLf & "Consulte la Conexión", vbCritical, "Conexión al S111"
		...
		//    Open spArchivo For Input As #ilArchivoDisponible
		//    ilRegistro = 0
		//' Almacena la información del Archivo en un arreglo de tipo de DatosDialogo
		//    While Not EOF(ilArchivoDisponible)
		//    'Lee el registro del archivo
		//        Line Input #ilArchivoDisponible, slRegistro
		//        ilRegistro = ilRegistro + 1
		//        ReDim dlgDialogo(1)
		//        slDialogo = Empty
		//    'Leer Layout
		//        mtclEjecutivo = ejefncGeneraEjecutivoMasivo(slRegistro, spSeparador)
		//
		//    'Asigna los valores correspondientes a dlgDialogo
		//        dlgDialogo(0).iFechaAlta = mtclEjecutivo.lEjeFechaAlta
		//
		//        dlgDialogo(0).sCuenta = Format(mtclEjecutivo.iEjePrefijo, "0000")
		//        dlgDialogo(0).sCuenta = dlgDialogo(0).sCuenta & Format(mtclEjecutivo.iGpoBanco, "00")
		//        dlgDialogo(0).sCuenta = dlgDialogo(0).sCuenta & Format(mtclEjecutivo.lEmpNum, String(ifncLongitudEmpresa(mtclEjecutivo.iEjePrefijo, mtclEjecutivo.iGpoBanco), "0"))
		//        dlgDialogo(0).sCuenta = dlgDialogo(0).sCuenta & Format(mtclEjecutivo.lEjeNum, String(ifncLongitudEjecutivo(mtclEjecutivo.iEjePrefijo, mtclEjecutivo.iGpoBanco), "0"))
		//        dlgDialogo(0).sCuenta = dlgDialogo(0).sCuenta & Format(mtclEjecutivo.iEjeRollOver, "0")
		//        dlgDialogo(0).sCuenta = dlgDialogo(0).sCuenta & Format(mtclEjecutivo.iEjeDigit, "0")
		//
		//        'dlgDialogo(0).iDiaCorte = mtclEjecutivo
		//        dlgDialogo(0).sNombreGrabacion = mtclEjecutivo.sEjeNomGrabado
		//        dlgDialogo(0).sNombre = Mid(mtclEjecutivo.sEjeNomGrabado, 1, InStr(1, mtclEjecutivo.sEjeNomGrabado, ",") - 1)
		//        dlgDialogo(0).sApellidoPaterno = Mid(mtclEjecutivo.sEjeNomGrabado, InStr(1, mtclEjecutivo.sEjeNomGrabado, ",") + 1, InStr(1, mtclEjecutivo.sEjeNomGrabado, "/") - InStr(1, mtclEjecutivo.sEjeNomGrabado, ",") - 1)
		//        dlgDialogo(0).sApellidoMaterno = Right(mtclEjecutivo.sEjeNomGrabado, Len(mtclEjecutivo.sEjeNomGrabado) - InStr(1, mtclEjecutivo.sEjeNomGrabado, "/"))
		//        dlgDialogo(0).sSexo = mtclEjecutivo.sEjeSexo
		//
		//        dlgDialogo(0).iSucTransmisora = mtclEjecutivo.sEjeSucTrans
		//        dlgDialogo(0).iSucPromotora = mtclEjecutivo.sEjeSucPromotora
		//        dlgDialogo(0).lLimiteCredito = mtclEjecutivo.lEjeLimiteCredito
		//        dlgDialogo(0).sDomicilio = mtclEjecutivo.sEjeDomCalle
		//        dlgDialogo(0).sColonia = mtclEjecutivo.sEjeColonia
		//        dlgDialogo(0).sPoblacion = mtclEjecutivo.sEjePoblacion
		//        dlgDialogo(0).sEstado = mtclEjecutivo.sEjeEstado
		//        dlgDialogo(0).iZonaPostal = mtclEjecutivo.iEjeZP
		//        dlgDialogo(0).lCP = mtclEjecutivo.lEjeCP
		//        'dlgDialogo(0).iASActi = mtclEjecutivo.sEjeActiSubacti
		//        dlgDialogo(0).lTelParticular = mtclEjecutivo.sEjeTelefonoDomicilio
		//        dlgDialogo(0).lTelOficina = mtclEjecutivo.sEjeTelefonoOficina
		//        dlgDialogo(0).lExtension = mtclEjecutivo.sEjeExtension
		//        dlgDialogo(0).sRFC = mtclEjecutivo.sEjeRFC
		//        dlgDialogo(0).iGeneraPinPlastico = mtclEjecutivo.iEjeGenPinPla
		//        dlgDialogo(0).sTipoCta = mtclEjecutivo.sEjeTipoCuenta
		//        dlgDialogo(0).iSkipPayment = mtclEjecutivo.iSkipPaymentFlag
		//        dlgDialogo(0).iIdTablaMCC = mtclEjecutivo.iTablaMCC
		//        dlgDialogo(0).dLimiteDisposiciones = mtclEjecutivo.lEjeLimDisEfec
		//        dlgDialogo(0).sCuentaReferencia = mtclEjecutivo.sEjeCuentaPadre
		//        dlgDialogo(0).sTipoFacturacion = mtclEjecutivo.sTipoFacturacion
		//        dlgDialogo(0).iNacionalidad = mtclEjecutivo.iNacionalidad
		//
		//        Select Case ipTipoAlta
		//'        Case "Empresa"
		//'        'Construye Diálogo
		//'            Dialogo slDialogo, dlgDialogo(0)
		//'        'Envía diálogo y si es exitoso realiza el alta
		//'            If bfncEnviaDialogo(cnxConexion, 333999, slDialogo, slEnviaRut, dlgDialogo(0).sCuenta, ejelEjecutivo) = True Then
		//'            'armar spAlta de empresa
		//'            'Alta tarjetahabiente 0
		//'            Else
		//'
		//'            End If
		//        Case "Ejecutivo"
		//            'Construye Diálogo
		//            Dialogo slDialogo, dlgDialogo(0)
		//            'Construye Datos básicos del ejecutivo
		//
		//            'Envía diálogo
		//            If bfncEnviaDialogo(cnxConexion, 333999, slDialogo, slEnviarut, dlgDialogo(0).sCuenta, ejelEjecutivo) = True Then
		//                'Armar sp para alta de tarjetahabiente
		//                'Armar los mensajes que se deriven del diálogo, para almacenarlo en la tabla de Altas Masivas
		//                ilAltaStatus = ifncAltaEjecutivo(mtclEjecutivo, slEnviarut, slEnviarut, 0)
		...
		//            End If
		//        End Select
		//        'Escribir el LOG para registro histórico de las altas
		//    Wend
		//
		//Cierra:
		//    Close #ilArchivoDisponible
		//    Screen.MousePointer = vbDefault
		//Exit Function
		//
		//ErrLayOutAltaS111S016:
		//    Select Case Err.Number
		//    Case 55
		//        GoTo Cierra
		//    End Select
		//    prObtenError "LayOutAltaS111S016"
		//End Function
		...
		//UPGRADE_NOTE: (7001) The following declaration (X) seems to be dead code
		//private void  X()
		//{
				//Campos del Catálogo
				//Domicilio Fiscal
				...
				//Alta de empresa
				//Si Alta de la empresa
				//   Obtener Empresa Consecutiva
				//   Leer parámetros de PGS
				//   Si se realiza el alta, se actualzan las firmas y los datos de los representantes
				//       y se coloca el consecutivo de ejecutivos =1
				//Actualiza las firmas de representante 1,2,3
				//Alta del primer tarjetahabiente con el Número de Ejecutivo 0
				//Obtiene la cuenta del ejecutivo 0
				//Prepara el diálogo para dar de alta el ejecutivo 0 (CERO) en el S111
				//Si exitoso el envío del dialogo para efectuar la transacción al 111 y 016
				//Alta en Sybase
				//Fin de Alta de tarjetahabiente
				//Actualiza el credito acumulado del grupo
				//Llama a la rutina que creará el directorio de la empresa
				//
				//si se realiza cambio de la empresa  (Domicilio de envío)
				...
				//Si es exitoso Actualiza el ejecutivo correspondiente a la empresa
				//Firma 1
				//Inicia el proceso de guardar las firmas
		...
		*/
        ...
        ///<!--
        /// Autor: LMHR & CMET
        /// Fecha de creacion: 27/12/2017
        /// Fecha de ultima modificacion: 27/12/2017-->
