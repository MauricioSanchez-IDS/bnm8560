...
namespace TCc430
{
	class IMAGEN
	{
			...
			try
			{
					...
					if (VBSQL.OpenConn(10) == 0)
					{
						...
						while(lDesplazamiento < lTamArch)
						{
                            ...
                            //AIS-1127 NGONZALEZ
                            yChunk = MidB(yImg, lDesplazamiento + 1, TAM_CHUNK);
        ...
        public static byte[] MidB(byte[] arraySource, int SourceIndex, int length)
        {
            ...
            if ((length - SourceIndex) > arraySource.Length)
                result = new byte[arraySource.Length - SourceIndex];
            else
                result = new byte[length - SourceIndex];
            ...
            {
                Array.Copy(arraySource, SourceIndex, result, 0, arraySource.Length - SourceIndex);
            }
            return result;
        }
