...
echo "sp_IntegracionFase1_HIH $1" > $DIR_TEMP/inteHIH1.sql
echo "go"       >> $DIR_TEMP/inteHIH1.sql
echo "EOF"      >> $DIR_TEMP/inteHIH1.sql
...
# 3. Crea la estructura de la tabla HIH
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteHIH1.sql  | grep -c "return status = 0" `
...
# 1. Se divide el archivo original EHIHMMDD en varios archivos con csplit
# 2. se genera un archivo por cada producto ejem: EHIHMMDD01, EHIHMMDD02, etc.
# 3. Se genera un formato de carga para cada uno de estos archivos a partir
#    del original HIH.fmt, ejem: HIH01.fmt, HIH02.fmt, etc.
...
do
  ...
  awk -v emp=$emp -v eje=$eje -f $DIR_CARGA/cambia.awk <$DIR_PARAM/HIH.fmt >$DIR_TEMP/HIH$consecutivo.fmt
done
...
# 1. hace un bulk copy por cada uno de los archi: EHIHMMDD01, EHIHMMDD02, etc.
# 3. considera los formatos correspondientes:  HIH01.fmt, HIH02.fmt, etc.
cont2=0
...
#bcp $(base.sh)..CARHIH01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/HIH${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
bcp64 $(base.sh)..CARHIH01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/HIH${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt -Jroman8
...
echo "sp_IntegracionFase2_HIH $1" > $DIR_TEMP/inteHIH2.sql
echo "go"       >> $DIR_TEMP/inteHIH2.sql
echo "EOF"      >> $DIR_TEMP/inteHIH2.sql

#---------- Ejecuta el Stored Procedure de 2a. Fase de Integracisn, que incluye:
#1. Si existe la tabla de PRODUCCION MTCHIH01, la borra
#2. Renombra la de CARGA por la de PRODUCCION MTCHIH01
#3. Crea los mndices y las llaves primarias o secundarias, si existen
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteHIH2.sql  | grep -c "return status = 0" `

#borrar lo puso yuria mayo4  divide entre 100 campos intereses y comisiones HIH
