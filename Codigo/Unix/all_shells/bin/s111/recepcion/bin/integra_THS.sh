...
do
  ...
  #eje=`echo $resultado | cut -c6-6`
  #emp=`echo $resultado | cut -c7-7`
...
bcp $(base.sh)..CARTHS01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/THS${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt -Jroman8
#bcp $(base.sh)..CARTHS01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/THS${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
