...
echo "sp_IntegracionFase1_REN $1" > $DIR_TEMP/inteREN1.sql
echo "go"       >> $DIR_TEMP/inteREN1.sql
echo "EOF"      >> $DIR_TEMP/inteREN1.sql
...
# 3. Crea la estructura de la tabla REN
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteREN1.sql  | grep -c "return status = 0" `
...
# 1. Se divide el archivo original ERENMMDD en varios archivos con csplit
# 2. se genera un archivo por cada producto ejem: ERENMMDD01, ERENMMDD02, etc.
# 3. Se genera un formato de carga para cada uno de estos archivos a partir
#    del original REN.fmt, ejem: REN01.fmt, REN02.fmt, etc.
...
do
  ...
  awk -v emp=$emp -v eje=$eje -f $DIR_CARGA/cambia.awk <$DIR_PARAM/REN.fmt >$DIR_TEMP/REN$consecutivo.fmt
done
...
# 1. hace un bulk copy por cada uno de los archi: ERENMMDD01, ERENMMDD02, etc.
# 3. considera los formatos correspondientes:  REN01.fmt, REN02.fmt, etc.
cont2=0
...
#bcp $(base.sh)..CARREN01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/REN${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt
bcp64 $(base.sh)..CARREN01 in $DIR_ARCH2/${1}${consecutivo} -Q -f$DIR_TEMP/REN${consecutivo}.fmt -U$USUARIO -P$PASSWD -S$SERVIDOR -e$DIR_LOGS/error.txt -Jroman8
...
echo "sp_IntegracionFase2_REN $1" > $DIR_TEMP/inteREN2.sql
echo "go"       >> $DIR_TEMP/inteREN2.sql
echo "EOF"      >> $DIR_TEMP/inteREN2.sql

#---------- Ejecuta el Stored Procedure de 2a. Fase de Integracisn, que incluye:
#1. Si existe la tabla de PRODUCCION MTCREN01, la borra
#2. Renombra la de CARGA por la de PRODUCCION MTCREN01
#3. Crea los mndices y las llaves primarias o secundarias, si existen
proc_Res=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_TEMP/inteREN2.sql  | grep -c "return status = 0" `
