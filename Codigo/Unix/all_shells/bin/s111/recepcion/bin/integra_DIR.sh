...
echo "sp_IntegracionFase1_DIR $1" > $DIR_CARGA/inteDIR1.sh
echo "go"       >> $DIR_CARGA/inteDIR1.sh
echo "EOF"      >> $DIR_CARGA/inteDIR1.sh
...
proc_OK=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_CARGA/inteDIR1.sh  | grep -c "return status = 0" `
...
echo "sp_IntegracionFase2_DIR $1" > $DIR_CARGA/inteDIR2.sh
echo "go"       >> $DIR_CARGA/inteDIR2.sh
echo "EOF"      >> $DIR_CARGA/inteDIR2.sh
...
proc_OK=`isql -U$USUARIO -P$PASSWD -D$(base.sh) -S$SERVIDOR -i $DIR_CARGA/inteDIR2.sh  | grep -c "return status = 0" `
