...
*******************************************************************************/
long int rg_arregloCliente[10000][4];
...
extern "C" {
    ...
    void    catClie(int, int, char *, long int [10000][4]);
    void    catRpr(int, int, char *, long int [10000][13]);
    int        extraNumCli(int, int, int, long int [10000][4]);
...
int            line;
{
    ...
    {
        fprintf(stderr, "Mensaje: %ld, Nivel: %d, Estado: %d, ",
                msgno,
                severity,
                msgstate);
        if (strlen(srvname) > 0)
