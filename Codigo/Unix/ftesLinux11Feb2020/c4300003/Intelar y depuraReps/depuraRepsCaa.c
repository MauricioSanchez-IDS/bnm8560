...
//extern T_EMPRESA        empresas[5000];
//extern T_REPORTE        reportes[15000];
extern long int            rg_arregloCliente[1000][4];
    ...
    while ((result_code = dbresults(dbproc)) != NO_MORE_RESULTS) {
        if (result_code == SUCCEED) {
            ...
            while (dbnextrow(dbproc) != NO_MORE_ROWS) {
                strcpy(empresas[indice].prefijo_emp,(char *) prefijo_emp);
                strcpy(empresas[indice].banco_emp, (char *)banco_emp);
                strcpy(empresas[indice].numero_emp, (char *)numero_emp);
...
int            ibanco;
char        cempresas[5000];
long int    rg_iNumerosClientes[1000][4];
{
