...
/*  fprintf(stdout,"Voy a Iniciar Validacion de Argumentos \n"); */
...
/*  fprintf(stdout,"Voy a Cargas Variables de Argumentos \n");  */
...
/*  fprintf(stdout,"Voy a Case de ejecutar Reporte \n"); */

  switch ( NumRep )
  {
   case  2 :

        sprintf(cadSQL, " exec sp_ObtenDatosRep2 %d,%d,%d,'%s','%s' ",
                         atoi(Prefijo),atoi(Banco),atoi(Empresa),Unidad,sFecha);
        break;

   case  7 :

        sprintf(cadSQL, " exec sp_ObtenDatosRep7 %d,%d,%d,'%s','%s' ",
                         atoi(Prefijo),atoi(Banco),atoi(Empresa),Unidad,sFecha);
        break;
...
/*  fprintf(stdout, "Respuesta %d al ejecutar EjecutaSQL \n",ret ); */
  if ( ret != 0 ) {
     fprintf(stdout, "Error %d al ejecutar la consulta de Empresas del grupo \n",ret );
     return (-1);
  }

/*   fprintf(stdout,"aft SQL...\n"); */
