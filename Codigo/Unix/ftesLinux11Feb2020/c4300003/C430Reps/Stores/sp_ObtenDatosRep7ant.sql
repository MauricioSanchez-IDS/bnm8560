...
select @sql=@sql +  " select e.eje_prefijo, e.gpo_banco, e.emp_num, e.eje_num, eje_nombre, ths_act_saldo, ths_hue_mes_vencidos_num1,"
select @sql=@sql +  "  ths_hue_mes_vencidos_num2, ths_hue_mes_vencidos_num3, ths_hue_mes_vencidos_num4, "
select @sql=@sql +  "  ths_hue_mes_vencidos_num5, ths_hue_mes_vencidos_num6, "
select @sql=@sql +  "  (convert(money,ths_hue_mes_vencidos_num1) + convert(money,ths_hue_mes_vencidos_num2) + "
select @sql=@sql +  "   convert(money,ths_hue_mes_vencidos_num3 + ths_hue_mes_vencidos_num4) + "
select @sql=@sql +  "   convert(money,ths_hue_mes_vencidos_num5) + convert(money,ths_hue_mes_vencidos_num6))as total_past_due, "
...
select @sql=@sql +  " and ths_hue_mes_vencidos_num1 > 0 "
...
select @sql=@sql +  " select ' ' + convert(char(27),eje_nombre) + ' ' + convert(char(12),convert(money,ths_act_saldo)) + '   ' +" 
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num1))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num2))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num3))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num4))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num5))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),ths_hue_mes_vencidos_num6))),9) + '.00' + "
...
select @sql=@sql +  " insert #Salida values (' ')"
select @sql=@sql +  " insert #Salida (detalle) "
select @sql=@sql +  " select 'TOTAL FOR :                  ' + convert(char(12),convert(money,sum(ths_act_saldo))) + '   ' +" 
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num1)))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num2)))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num3)))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num4)))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num5)))),9) + '.00   ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),sum(ths_hue_mes_vencidos_num6)))),9) + '.00' + "
select @sql=@sql +  " char(10) + convert(char(35),@NomEmp) + replicate(char(9),10) + '       ' + "
select @sql=@sql +  " right('         '+(rtrim(convert(char(9),(sum(ths_hue_mes_vencidos_num1)+sum(ths_hue_mes_vencidos_num2)+"
select @sql=@sql +  " sum(ths_hue_mes_vencidos_num3)+sum(ths_hue_mes_vencidos_num4)+sum(ths_hue_mes_vencidos_num5)+"
select @sql=@sql +  " sum(ths_hue_mes_vencidos_num6))))),9) + '.00' "
select @sql=@sql +  " + char(10) + '  ' + char(10) from #mtcejeths "
