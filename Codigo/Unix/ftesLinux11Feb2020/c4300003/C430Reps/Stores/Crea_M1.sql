...
go
IF OBJECT_ID('dbo.M1') IS NOT NULL
    PRINT '<<< CREATED TABLE dbo.M1 >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE dbo.M1 >>>'
go
/*
 * Creating Indexes for Table dbo.M1
 */


CREATE CLUSTERED INDEX XIF18M1
    ON dbo.M1(eje_prefijo,gpo_banco,emp_num,eje_num) 
go
IF OBJECT_ID('dbo.XIF18M1') IS NOT NULL
    PRINT '<<< CREATED INDEX dbo.XIF18M1>>>'
ELSE
    PRINT '<<< FAILED CREATING INDEX dbo.XIF18M1>>>'
go
